...
int main(int nargs, char **argv)
{
    ...
    for (int file_i = 1; file_i < nargs; file_i++)
    {
        ...
        {
        	//#pragma omp parallel for shared(grey_image) private(r, g, b)	para bucle interno
...
#define PIXEL_GREY(x, y) (grey_image[(x) + (y)*width])
        ...
        {
        	//#pragma omp parallel for shared(edges)	para bucle interno
...
#define PIXEL_EDGES(x, y) (edges[(x) + (y)*width_edges]);
        ...
        if (TYPE_FILTER == MEDIAN)
        {
            ...
            {
            	//#pragma omp parallel for shared(edges_denoised) private(x, y, k, array)	para bucle interno
        ...
        // More classic gaussian filter
            ...
            for (int j = radius; j < height_edges - radius; j++) //Antes: for i {for j}
            {
            	//#pragma omp parallel for shared(edges_denoised) firstprivate(sum) private(x, y)	para bbucle interno
                for (int i = radius; i < width_edges - radius; i++)
                {
                    x = i - radius;
                    y = j - radius;
